...
// --- IMPORTAÇÃO DAS TABS ---
import { TabCatalogo } from "./_tabsDashBoard/TabCatalogo";
import { TabSazonalidade } from "./_tabsDashBoard/TabSazonalidade";
import { TabClientes } from "./_tabsDashBoard/TabClientes";
import { TabLucroReal } from "./_tabsDashBoard/TabLucroReal";
import { TabDevolucoes } from "./_tabsDashBoard/TabDevolucoes";
import { TabFaturamentoCanais } from "./_tabsDashBoard/TabFaturamentoCanais";
import { TabDespesas } from "./_tabsDashBoard/TabDespesas";
import { TabRelatorioHistorico } from "./_tabsDashBoard/TabRelatorioHistorico";
...
// ============================================================================
// COMPONENTE AUXILIAR: LINHA PEDIDO
// ============================================================================
// ============================================================================
// COMPONENTE AUXILIAR: LINHA PEDIDO (CORRIGIDO)
// ============================================================================
...
export function DashboardGestao({ pedidos, lojistaId }: { pedidos: Pedido[], lojistaId?: string }) {
  ...
  const inteligencia = useMemo(() => {
    const i = {
      faturamento: 0, faturamentoInternoPuro: 0, lucroReal: 0, custoTotal: 0, perdaDevolucao: 0, totalPedidosValidos: 0, 
      despesaFreteLojista: 0, despesasOperacionaisLojista: 0, despesasFixas: 0, despesasVariaveis: 0,
      ...
      // 🔥 AQUI ESTÁ A ESTRUTURA PARA O GRÁFICO:
    ...
    return i;
  }, [pedidos, canaisExternos, despesasLojista, dataInicio, dataFim, parseDataPedido, recursosLiberados]);
  useEffect(() => {
  ...
  const lista = [
    // 1. OPERACIONAL (Dia a dia)
    { id: "vendas", label: "VENDAS" },
    { id: "devolucoes", label: "🔄 DEVOLVIDOS" },
    
    // 2. GESTÃO (Saúde financeira atual)
    { id: "lucro", label: "💰 LUCRO REAL" },
    { id: "precificacao", label: "🧮 PRECIFICAÇÃO" },
    
    // 3. ESTRATÉGIA (O que não é rotina diária)
    { id: "sazonalidade", label: "SAZONALIDADE" },
    { id: "catalogo", label: "CATÁLOGO" },
    { id: "clientes", label: "CLIENTES" },

    // 4. VISÃO GERAL HISTÓRICA (O "Cérebro" do sistema no fim)
    { id: "historico", label: "📈 RELATÓRIO HISTÓRICO" }
  ];

  // Adições condicionais do plano
  if (recursosLiberados.temCanaisRenda) lista.push({ id: "canais", label: "📦 CANAIS DE RENDA" });
  if (recursosLiberados.temDespesas) lista.push({ id: "despesas", label: "💸 DESPESAS" });
...
>
  🧪 Inserir Dados de Teste
      ...
      {/* 🚀 HUB DE GAMIFICAÇÃO: BARRA DE METAS */}
      <div style={styles.metaContainer}>
        ...
        <p style={styles.metaMotivationText}>
          {progressoMeta >= 100 
            ? "🔥 Incrível! Você explodiu sua meta deste mês! Parabéns! 🚀" 
            : `Faltam ${formatarMoeda(Math.max(0, metaFaturamento - inteligencia.faturamento))} para bater sua meta de faturamento. Vamos pra cima! ⚡`}
        </p>
      </div>

      <header style={styles.header}>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px'}}>
          <h1 style={{margin: 0, color: '#2c3e50'}}>📈 Gestão de Lucratividade</h1>
          <button onClick={() => router.back()} style={styles.btnVoltar}>⬅️ Voltar</button>
        </div>
        
        {/* FILTROS INTEGRADOS UNIFICADOS NO TOPO */}
        <div style={styles.filtrosCard}>
          ...
          <button onClick={() => { const h = new Date().toISOString().split('T')[0]; setDataInicio(h); setDataFim(h); }} style={styles.btnAtalho}>Hoje</button>
          <button onClick={() => { setBuscaNome(""); setDataInicio(""); setDataFim(""); }} style={styles.btnLimpar}>Limpar</button>
        </div>
        
        {/* CONTAINER DE TAB BAR ATUALIZADO COM GRID-WRAP CONTRA SCROLL */}
      ...
      <section style={styles.section}>
        <div style={styles.abaHeader}>
          ...
          <button style={styles.infoTooltip} onClick={() => alert(INFO_ABAS[abaAtiva] || "Visão Geral")}>ℹ️</button>
        </div>
    ...
    <tbody>
      {/* Aplicando o filtro apenas aqui na visualização */}
      {dadosFiltradosBusca
        .filter(p => p.status?.toLowerCase() === 'concluído' && !p.devolvido)
        .map(p => (
          <LinhaPedido 
            key={p.id} 
            pedido={p} 
        ...
        {abaAtiva === 'clientes' && <TabClientes clientesEstrela={clientesEstrelaFiltrados} formatarMoeda={formatarMoeda} styles={styles} />}
    ...
    // 🔥 Mude aqui de evolucaoMensal para o nome que definimos no objeto:
  ...
  />
        ...
        {abaAtiva === 'devolucoes' && <TabDevolucoes dadosFiltradosBusca={dadosFiltradosBusca} formatarDataExibicao={formatarDataExibicao} formatarMoeda={formatarMoeda} alternarDevolucao={alternarDevolucao} styles={styles} />}
      </section>
    </div>
  );
}
