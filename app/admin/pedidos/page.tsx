...
interface ItemCarrinho {
  id?: string; qty?: number; quantidade?: number; nome?: string; title?: string; variacao?: string; preco?: number; precisaFrete?: boolean; respostasFormatadas?: Record<string, any>; personalizacao?: Record<string, any>;
}
interface EnderecoPedido { rua?: string; numero?: string; bairro?: string; cidade?: string; city?: string; uf?: string; cep?: string; }
interface FinanceiroPedido { metodo?: string; dstransportadoraId?: string; freteGratis?: boolean; }
interface Pedido {
  id: string; numeroPedido?: string | number; numero?: string | number; cliente: string | { nome?: string; [key: string]: any }; endereco?: EnderecoPedido; itens: ItemCarrinho[]; financeiro?: FinanceiroPedido; pago: boolean; status: string; etiquetaGerada?: boolean; dataGeracaoEtiqueta?: string | null; data?: string; retirada?: boolean; retirarNaLoja?: boolean; formaEntrega?: string;
}
...
export default function GestaoPedidos({ 
  ...
  const temPedidosPendentes = useMemo(() => {
    return localPedidos.some(p => p.etiquetaGerada && p.statusEtiqueta === 'pendente');
  }, [localPedidos]);
...
const resetarStatusErro = (pedido: Pedido) => {
  ...
  const tentarPagamento = async (pedido: Pedido) => {
    try {
      const res = await fetch("/api/frete/tentar-pagamento", { 
        ...
        body: JSON.stringify({ lojistaId: lojistaIdApp, pedidoId: pedido.id, transportadoraId: pedido.financeiro?.transportadoraId }) 
      });
  ...
  for (const pedido of pedidosParaGerar) {
    try {
      ...
      setModalProgresso(prev => ({
        ...prev,
        itens: prev.itens.map(i => i.id === pedido.id ? { ...i, status: 'erro', mensagem: sucesso ? "" : (data.errors?.find((e: any) => e.pedido === pedido.id)?.message || "Erro no checkout")
        } : i)
        }));
    }
  }
