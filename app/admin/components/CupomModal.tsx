  ...
  return (
    <div style={modalStyles.overlay} onClick={onClose}>
      <div style={modalStyles.modal} onClick={e => e.stopPropagation()}>
        ...
        <button style={modalStyles.btnAdd} onClick={() => {
           const nome = prompt("Código do cupom:")?.toUpperCase().trim();
           if (nome) setCupons({ ...cupons, [nome]: { valor: 0, tipo: 'fixo', ativo: true } });
        }}>+ Novo Cupom</button>
        
        <div style={modalStyles.itemList}>
          {Object.keys(cupons).map(nome => (
            <div key={nome} style={modalStyles.cupomRow}>
              <div style={{ flex: 1, fontWeight: 'bold', fontSize: '13px' }}>{nome}</div>
              <input 
                type="number" 
                style={{ ...modalStyles.input, width: '60px' }} 
                value={cupons[nome].valor} 
                onChange={e => setCupons({...cupons, [nome]: {...cupons[nome], valor: Number(e.target.value)}})}
              />
              <button onClick={() => {
                const n = {...cupons}; delete n[nome]; setCupons(n);
