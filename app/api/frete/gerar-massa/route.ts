...
export async function POST(request: Request) {
  try {
    ...
    if (!lojistaSnap.exists) return NextResponse.json({ error: "Lojista não encontrado." }, { status: 404 });
    ...
    for (const p of orders) {
      const itensFisicos = (p.itens || []).filter((item: any) => item.precisaFrete !== false);
      ...
      const pedidoRef = db.collection("lojistas").doc(lojistaId).collection("pedidos").doc(p.id);
      ...
      const payloadCart = {
        ...
        from: {
          ...
          address: dadosLoja.ruaOrigem,
          number: dadosLoja.numeroOrigem,
        ...
        to: { 
          name: p.cliente.nmNome || p.cliente.nome, 
        ...
        options: { insurance_value: totalFisico.valor, non_commercial: true, platform: "FestaEmTopo", note: p.financeiro?.metodo }
      };
      ...
      if (!cartRes.ok) { 
        errors.push({ pedido: p.id, message: cartData.message || "Erro no carrinho" });
        continue; 
      }
      ...
      if (checkoutRes.ok) {
        ...
        console.error("Detalhes do erro:", JSON.stringify(checkoutData, null, 2));
        console.error("---------------------------------------");

        let msgErro = checkoutData.error || checkoutData.message || "Erro desconhecido";
        if (typeof msgErro === 'string' && msgErro.toLowerCase().includes("saldo")) {
            msgErro = "Saldo insuficiente no Melhor Envio. Adicione créditos para continuar.";
        }
        await pedidoRef.update({ 
