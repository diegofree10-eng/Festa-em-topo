...
export const executarFluxoPedido = async ({
  ...
  try {
    ...
    const dadosDoPedidoParaSalvar = {
      ...
      mesAno: new Date().toISOString().substring(0, 7), // Facilitador para Dashboards

      // Cliente (nm = nome, ds = dados/string)
      cliente: { 
      ...
      // Financeiro (vl = valor, is = boolean, ds = dados)
      financeiro: { 
      ...
      // Logística (is = boolean, ds = dados)
      logistica: {
        isRetirada: logistica.formaEnvio === 'retirada',
        dsFormaEntrega: logistica.formaEnvio
      },

      itens: itensFormatados,

      // --- CAMPOS LEGADOS (MANTIDOS PARA COMPATIBILIDADE) ---
      // Enquanto você não atualizar todas as páginas, estes campos garantem que o admin atual continue lendo
...
📦 *ITENS:*
${safeCart.map((i: any) => `• ${i.qty}x ${i.nome}`).join('\n')}

💰 *TOTAL:* R$ ${totalGeral.toFixed(2).replace('.', ',')}
Acesse seu painel para processar este pedido!`;

    const url = `https://wa.me/${whatsappNumero.replace(/\D/g, "")}?text=${encodeURIComponent(msg)}`;
